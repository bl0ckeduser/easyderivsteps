...
/* `hack' is a global variable accessed by other .c files.
 * when set to 1, disables transformations that
 * do not change the rendered appearence of expressions.
 * used to prevent cases where lines are repeated.
 * This is probably the only thing not in symdiff.
 */
int hack = 0;
...
#ifdef OPT_DEBUG
	#define return(X) { printf("D: leaving optimizer at line %d\n", \
					 		__LINE__); return(X); }
#endif

#ifdef JSDEBUG
#define return(X) { debugopt(__LINE__); return(X); }
#endif
...
{
	char buffer[128];
	sprintf(buffer, "alert('optline %d')", n);
...
int special(exp_tree_t *et)
{
	...
	if (et->head_type == FUNC
		...
		write_tree_latex(et, buff);
		*step_desc = 0;
		sprintf(step_desc, "Apply sum rule on $");
		strcat(step_desc, buff);
		strcat(step_desc, " $");
		puts(step_desc);
		free(buff);
		memcpy(et, new_ptr, sizeof(exp_tree_t));
